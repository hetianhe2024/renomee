...
其中 `/en/latest/` 是 ReadTheDocs 的项目语言和版本路径，不一定代表站内内容只能是英文。考虑到当前文档体量、维护成本和引流目标，暂时不单独拆分多个 ReadTheDocs 项目。
...
英文核心文档：
https://renomeeai.readthedocs.io/en/latest/en/
```

这个结构不是最标准的多语言 URL，但对当前阶段是成本最低、落地最快的方案。SEO 层面主要依赖页面内容、标题、描述、语言声明、内链和 sitemap，路径中外层 `/en/` 不会造成明显损失。

### 执行原则

- 中文文档保持完整，继续作为主文档维护。
- 英文文档只做核心内容，用于承接英文官网和搜索流量。
- 英文页面不要求和中文页面一一对应，避免全量翻译带来的维护负担。
- 中文首页和英文首页互相链接，保证用户和搜索引擎能发现两个语言入口。
- 英文页面单独编写 title、description 和正文，不做机械翻译。
...
docs/en/basic-usage.md
docs/en/faq.md
```

暂不优先翻译 `advanced-features.md`、`changelog.md` 和 `privacy.md`，除非后续英文访问数据证明有需求。
